...
SPI_HandleTypeDef* WIFININA_SPIWIFI;
...
void SpiDrv::begin() {
  ...
  */
  ...
  if (WIFININA_SLAVEGPIO0 >= 0) {
    ...
    HAL_GPIO_WritePin(WIFININA_SLAVEGPIO0_PORT, WIFININA_SLAVEGPIO0, HIGH);
  }

  HAL_GPIO_WritePin(WIFININA_SLAVESELECT_PORT, WIFININA_SLAVESELECT, HIGH);
  HAL_GPIO_WritePin(WIFININA_SLAVERESET_PORT, WIFININA_SLAVERESET, inverted_reset ? HIGH : LOW);
  delay(10);
  HAL_GPIO_WritePin(WIFININA_SLAVERESET_PORT, WIFININA_SLAVERESET, inverted_reset ? LOW : HIGH);
  delay(750);

  if (WIFININA_SLAVEGPIO0 >= 0) {
    // digitalWrite(WIFININA_SLAVEGPIO0, LOW);
    // pinMode(WIFININA_SLAVEGPIO0, INPUT_PULLUP);
    pinMode(WIFININA_SLAVEGPIO0_PORT, WIFININA_SLAVEGPIO0, INPUT, PULLUP);
  }
...
void SpiDrv::end() {
  HAL_GPIO_WritePin(WIFININA_SLAVERESET_PORT, WIFININA_SLAVERESET,
                    inverted_reset ? HIGH : LOW);

  pinMode(WIFININA_SLAVESELECT_PORT, WIFININA_SLAVESELECT, INPUT, NOPULL);
...
void SpiDrv::spiSlaveSelect() {
  ...
  HAL_GPIO_WritePin(WIFININA_SLAVESELECT_PORT, WIFININA_SLAVESELECT, LOW);
  ...
  for (unsigned long start = millis();
       (HAL_GPIO_ReadPin(WIFININA_SLAVEREADY_PORT, WIFININA_SLAVEREADY) !=
        HIGH) &&
       (millis() - start) < 5;)
    ;
}
...
char SpiDrv::spiTransfer(volatile char data) {
  char rxData = SPITransfer(data);

  DELAY_TRANSFER();

  return rxData;  // return the received byte
}
...
#define waitSlaveReady() \
  (HAL_GPIO_ReadPin(WIFININA_SLAVEREADY_PORT, WIFININA_SLAVEREADY) == LOW)
#define waitSlaveSign() \
  (HAL_GPIO_ReadPin(WIFININA_SLAVEREADY_PORT, WIFININA_SLAVEREADY) == HIGH)
#define waitSlaveSignalH()                                                  \
  while (HAL_GPIO_ReadPin(WIFININA_SLAVEREADY_PORT, WIFININA_SLAVEREADY) != \
         HIGH) {                                                            \
  }
 ...
 */
/*|   8 bit   | 1bit | 7bit |  8bit   |  8bit   |   8bit    | nbytes | .. |
 * 8bit  | */
/*|___________|______|______|_________|_________|___________|________|____|_________|
 */
...
int SpiDrv::available() {
  if (WIFININA_SLAVEGPIO0 >= 0) {
    return (HAL_GPIO_ReadPin(WIFININA_SLAVEGPIO0_PORT, WIFININA_SLAVEGPIO0) ==
            HIGH);
  }
  return true;
}
