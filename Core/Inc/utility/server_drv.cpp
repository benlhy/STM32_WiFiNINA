...
void ServerDrv::startServer(uint16_t port, uint8_t sock, uint8_t protMode) {
  ...
  uint8_t _data = 0;
  uint8_t _dataLen = 0;
  if (!SpiDrv::waitResponseCmd(START_SERVER_TCP_CMD, PARAM_NUMS_1, &_data,
                               &_dataLen)) {
...
void ServerDrv::startServer(uint32_t ipAddress, uint16_t port, uint8_t sock,
                            uint8_t protMode) {
  WAIT_FOR_SLAVE_SELECT();
  // Send Command
...
void ServerDrv::startClient(uint32_t ipAddress, uint16_t port, uint8_t sock,
  ...
  uint8_t _data = 0;
  uint8_t _dataLen = 0;
  if (!SpiDrv::waitResponseCmd(START_CLIENT_TCP_CMD, PARAM_NUMS_1, &_data,
                               &_dataLen)) {
    WARN("error waitResponse");
  }
  SpiDrv::spiSlaveDeselect();
...
void ServerDrv::startClient(const char *host, uint8_t host_len,
                            uint32_t ipAddress, uint16_t port, uint8_t sock,
                            uint8_t protMode) {
  WAIT_FOR_SLAVE_SELECT();
  // Send Command
...
void ServerDrv::stopClient(uint8_t sock) {
  ...
  uint8_t _data = 0;
  uint8_t _dataLen = 0;
  if (!SpiDrv::waitResponseCmd(STOP_CLIENT_TCP_CMD, PARAM_NUMS_1, &_data,
                               &_dataLen)) {
    WARN("error waitResponse");
  }
  SpiDrv::spiSlaveDeselect();
...
uint8_t ServerDrv::getServerState(uint8_t sock) {
  ...
  uint8_t _data = 0;
  uint8_t _dataLen = 0;
  if (!SpiDrv::waitResponseCmd(GET_STATE_TCP_CMD, PARAM_NUMS_1, &_data,
                               &_dataLen)) {
    WARN("error waitResponse");
  }
  SpiDrv::spiSlaveDeselect();
...
uint8_t ServerDrv::getClientState(uint8_t sock) {
  ...
  uint8_t _data = 0;
  uint8_t _dataLen = 0;
  if (!SpiDrv::waitResponseCmd(GET_CLIENT_STATE_TCP_CMD, PARAM_NUMS_1, &_data,
                               &_dataLen)) {
    WARN("error waitResponse");
  }
  SpiDrv::spiSlaveDeselect();
...
uint16_t ServerDrv::availData(uint8_t sock) {
  ...
  SpiDrv::waitResponseCmd(AVAIL_DATA_TCP_CMD, PARAM_NUMS_1, (uint8_t *)&len,
                          &_dataLen);
...
uint8_t ServerDrv::availServer(uint8_t sock) {
  ...
  SpiDrv::waitResponseCmd(AVAIL_DATA_TCP_CMD, PARAM_NUMS_1, (uint8_t *)&socket,
                          &_dataLen);
...
bool ServerDrv::insertDataBuf(uint8_t sock, const uint8_t *data,
                              uint16_t _len) {
  WAIT_FOR_SLAVE_SELECT();
  // Send Command
...
uint16_t ServerDrv::sendData(uint8_t sock, const uint8_t *data, uint16_t len) {
  ...
  uint16_t _data = 0;
  uint8_t _dataLen = 0;
  if (!SpiDrv::waitResponseData8(SEND_DATA_TCP_CMD, (uint8_t *)&_data,
                                 &_dataLen)) {
    WARN("error waitResponse");
  }
  SpiDrv::spiSlaveDeselect();
...
uint8_t ServerDrv::checkDataSent(uint8_t sock) {
  ...
  do {
    ...
    // Wait for reply
    if (!SpiDrv::waitResponseCmd(DATA_SENT_TCP_CMD, PARAM_NUMS_1, &_data,
                                 &_dataLen)) {
