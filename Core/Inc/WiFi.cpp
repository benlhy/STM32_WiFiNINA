...
#include "WiFi.h"
#include "utility/wifinina.h"
#include "utility/wifi_drv.h"

extern SPI_HandleTypeDef* WIFININA_SPIWIFI;
...
void WiFiClass::setPins(int8_t cs, int8_t ready, int8_t reset, int8_t gpio0,
		SPI_HandleTypeDef *hspi) {
  WIFININA_SLAVESELECT = cs;
  WIFININA_SLAVEREADY = ready;
  WIFININA_SLAVERESET = reset;
  WIFININA_SLAVEGPIO0 = gpio0;
  WIFININA_SPIWIFI = hspi;
}

void WiFiClass::init() { WiFiDrv::wifiDriverInit(); }

const char* WiFiClass::firmwareVersion() { return WiFiDrv::getFwVersion(); }
...
uint8_t WiFiClass::beginAP(const char* ssid) { return beginAP(ssid, 1); }
...
uint8_t WiFiClass::beginAP(const char* ssid, const char* passphrase,
  ...
  // set passphrase
  if (WiFiDrv::wifiSetApPassphrase(ssid, strlen(ssid), passphrase,
                                   strlen(passphrase), channel) != WL_FAILURE) {
    do {
      delay(WL_DELAY_START_CONNECTION);
      status = WiFiDrv::getConnectionStatus();
    } while (((status == WL_IDLE_STATUS) || (status == WL_SCAN_COMPLETED)) &&
...
void WiFiClass::setHostname(const char* name) { WiFiDrv::setHostname(name); }

int WiFiClass::disconnect() { return WiFiDrv::disconnect(); }

void WiFiClass::end(void) { WiFiDrv::wifiDriverDeinit(); }
...
const char* WiFiClass::SSID() { return WiFiDrv::getCurrentSSID(); }
...
int32_t WiFiClass::RSSI() { return WiFiDrv::getCurrentRSSI(); }
...
int8_t WiFiClass::scanNetworks() {
  ...
  if (WiFiDrv::startScanNetworks() == WL_FAILURE) return WL_FAILURE;
  do {
    delay(200);
...
unsigned long WiFiClass::getTime() { return WiFiDrv::getTime(); }

void WiFiClass::lowPowerMode() { WiFiDrv::setPowerMode(1); }

void WiFiClass::noLowPowerMode() { WiFiDrv::setPowerMode(0); }
